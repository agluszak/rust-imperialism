...
// Helper function to run pathfinding with QueryState
fn find_path_in_world(
    world: &mut World,
    start: TilePos,
    goal: TilePos,
    ...
    world
        .run_system_once(move |tile_query: Query<(&TileType, &TilePos)>| {
...
// Helper function to calculate path cost with QueryState
fn calculate_path_cost_in_world(
    world: &mut World,
    path: &[TilePos],
    tile_storage: &TileStorage,
) -> u32 {
    ...
    world
        .run_system_once(move |tile_query: Query<(&TileType, &TilePos)>| {
            PathfindingSystem::calculate_path_cost_with_combined_query(
                &path_vec,
                &tile_query,
                &tile_storage,
            )
...
fn test_pathfinding_straight_line() {
    ...
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 5, 5);
    ...
    let mut tile_query = world.query::<(&TileType, &TilePos)>();

    let path = find_path_in_world(&mut world, start, goal, &tilemap_size, &_tile_storage);
...
#[test]
fn test_pathfinding_no_path_to_unreachable() {
    let mut world = create_test_world();
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 5, 5);
    let start = TilePos { x: 0, y: 0 };
...
fn test_pathfinding_same_position() {
    let mut world = create_test_world();
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 3, 3);
    let pos = TilePos { x: 1, y: 1 };
    let tilemap_size = TilemapSize { x: 3, y: 3 };

    let mut tile_query = world.query::<(&TileType, &TilePos)>();

    let path = find_path_in_world(&mut world, pos, pos, &tilemap_size, &_tile_storage);
...
fn test_path_cost_calculation() {
    ...
    let cost = calculate_path_cost_in_world(&mut world, &path, &_tile_storage);
...
#[test]
fn test_path_cost_empty_path() {
    let mut world = create_test_world();
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 3, 3);
    let empty_path = vec![];
    let mut tile_query = world.query::<(&TileType, &TilePos)>();

    let cost = calculate_path_cost_in_world(&mut world, &empty_path, &_tile_storage);

    assert_eq!(cost, 0);
}

#[test]
fn test_path_cost_single_position() {
    let mut world = create_test_world();
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 3, 3);
    let single_path = vec![TilePos { x: 1, y: 1 }];
    let mut tile_query = world.query::<(&TileType, &TilePos)>();

    let cost = calculate_path_cost_in_world(&mut world, &single_path, &_tile_storage);
...
fn test_pathfinding_diagonal_movement() {
    let mut world = create_test_world();
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 4, 4);
    let start = TilePos { x: 0, y: 0 };
...
#[test]
fn test_pathfinding_around_water_barrier() {
    let mut world = create_test_world();
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 7, 5);
    let start = TilePos { x: 0, y: 2 };
    let goal = TilePos { x: 6, y: 2 };
    let tilemap_size = TilemapSize { x: 7, y: 5 };

    // Create a partial vertical water barrier (leaving gaps at top and bottom)
    for y in 1..4 {
...
#[test]
fn test_path_cost_with_mixed_terrain() {
    let mut world = create_test_world();
    let (_tilemap_entity, mut _tile_storage) = create_test_tilemap(&mut world, 4, 1);
