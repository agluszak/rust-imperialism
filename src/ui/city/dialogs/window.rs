...
/// Handle close button clicks (Input Layer)
pub fn handle_dialog_close_buttons(
    interactions: Query<(&Interaction, &DialogCloseButton), Changed<Interaction>>,
    mut close_writer: MessageWriter<CloseBuildingDialog>,
) {
    for (interaction, button) in interactions.iter() {
        if *interaction == Interaction::Pressed {
            close_writer.write(CloseBuildingDialog {
