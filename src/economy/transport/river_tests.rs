...
fn setup_app() -> (App, Entity) {
    ...
    app.world_mut()
        .entity_mut(tile_storage_entity)
        .insert(tile_storage);
...
fn test_is_ocean_tile_open_ocean() {
    ...
    let mut system_state: SystemState<(Query<&TileStorage>, Query<&TerrainType>)> =
    ...
    // Should be Ocean
    assert!(
        is_ocean_tile(center, storage, &tile_query),
        "Open ocean should be detected as ocean"
    );
}
...
fn test_is_ocean_tile_river() {
    ...
    assert!(
        !is_ocean_tile(center, storage, &tile_query),
        "Straight river should NOT be ocean"
    );
}
...
fn test_is_ocean_tile_coast() {
    ...
    assert!(
        is_ocean_tile(center, storage, &tile_query),
        "Coast should be detected as ocean"
    );
}
...
fn test_is_ocean_tile_confluence() {
    ...
    assert!(
        !is_ocean_tile(center, storage, &tile_query),
        "River confluence should NOT be ocean"
    );
}
