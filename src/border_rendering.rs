                    ...
                    for neighbor_hex in tile_hex.all_neighbors() {
                        if let Some(neighbor_pos) = neighbor_hex.to_tile_pos() {
                            ...
                            if let Some(neighbor_entity) = tile_storage.get(&neighbor_pos) {
                                if let Ok(neighbor_prov) = tile_provinces.get(neighbor_entity) {
                                    ...
                                    if tile_prov.province_id != neighbor_prov.province_id {
                                        ...
                                        if is_international {
                                            ...
                                            gizmos.line_2d(line_start + tile_offset, line_end + tile_offset, tile_color);
                                            let tile_offset2 = direction * 1.5;
                                            gizmos.line_2d(line_start + tile_offset2, line_end + tile_offset2, tile_color);

                                            // Neighbor nation's side (offset towards neighbor)
                                            let neighbor_offset = direction * -2.5;
                                            gizmos.line_2d(line_start + neighbor_offset, line_end + neighbor_offset, neighbor_color);
                                            let neighbor_offset2 = direction * -1.5;
                                            gizmos.line_2d(line_start + neighbor_offset2, line_end + neighbor_offset2, neighbor_color);

                                            // Center line for definition
                                            gizmos.line_2d(line_start, line_end, Color::srgb(0.3, 0.3, 0.3));
                                        } else {
