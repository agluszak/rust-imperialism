...
fn setup_terminal_log(mut terminal_log: ResMut<TerminalLog>) {
    ...
    for i in 1..=30 {
        terminal_log.add_message(format!("Test message {} - this is a longer message to demonstrate scrolling behavior", i));
    }
}
...
fn update_scrollbar(
    ...
    mut thumb_query: Query<&mut Node, (With<ScrollbarThumb>, Without<ScrollableTerminal>, Without<ScrollbarDragStart>)>,
    terminal_log: Res<TerminalLog>,
) {
...
fn handle_scrollbar_drag(
    ...
    mut scrollable_query: Query<(&mut ScrollPosition, &Node, &ComputedNode), With<ScrollableTerminal>>,
    ...
    if mouse_button_input.just_pressed(MouseButton::Left) {
        for track_cursor in track_query.iter() {
            if let Some(pos) = track_cursor.normalized {
                if let Ok((mut scroll_position, _node, computed)) = scrollable_query.single_mut() {
                    let content_size = computed.content_size();
                    let visible_size = computed.size();
    ...
    for (thumb_entity, cursor_position) in thumb_query.iter() {
            ...
            if let Some(pos) = cursor_position.normalized {
                if let Ok((scroll_position, _, _)) = scrollable_query.single() {
                    commands.entity(thumb_entity).insert(ScrollbarDragStart {
                        position: pos,
                        scroll_position: Vec2::new(scroll_position.offset_x, scroll_position.offset_y),
                    });
                }
            }
        }
        ...
        if mouse_button_input.pressed(MouseButton::Left) {
            if let Ok(drag_start) = drag_query.get(thumb_entity) {
                if let Some(current_pos) = cursor_position.normalized {
                    ...
                    if let Ok((mut scroll_position, _node, computed)) = scrollable_query.single_mut() {
                        ...
                        if content_size.y > visible_size.y {
                            ...
                            scroll_position.offset_y = (drag_start.scroll_position.y + scroll_delta)
                                .max(0.0)
                                .min(max_scroll);
                        }
                    }
