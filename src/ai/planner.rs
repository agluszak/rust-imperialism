...
fn assign_civilians_to_goals(
    ...
    // - all friendly civilian positions
    // The tracker is updated incrementally as tasks are assigned.
    let mut tracker = ReservationTracker::from_world_state(nation, snapshot);
        ...
        for civilian in nation.available_civilians() {
            ...
            let task_opt = match goal {
                ...
                NationGoal::ConnectDepot { tile, .. }
                    if civilian.kind == CivilianKind::Engineer =>
                {
                    plan_engineer_rail_task(nation, snapshot, &tracker, civilian.position, *tile)
                }
                    ...
                    if civilian.kind == CivilianKind::Prospector =>
                {
                    if civilian.position == *tile || is_adjacent(civilian.position, *tile) {
                        ...
                        plan_move_toward_goal(nation, &tracker, civilian.position, *tile)
                    }
                }
            ...
            if let Some(task) = task_opt {
                ...
                let distance = score_task_distance(civilian.position, goal, &task);
...
fn plan_move_toward_goal(
    nation: &NationSnapshot,
    tracker: &ReservationTracker,
    from: TilePos,
    goal: TilePos,
) -> Option<CivilianTask> {
    if tracker.is_occupied(goal) {
        return None;
    }

    find_step_toward(from, goal, &nation.owned_tiles, tracker)
        .map(|step| CivilianTask::MoveTo { target: step })
}

fn score_task_distance(civilian_pos: TilePos, goal: &NationGoal, task: &CivilianTask) -> u32 {
    match goal {
        NationGoal::ProspectTile { tile, .. } | NationGoal::ImproveTile { tile, .. } => {
            civilian_pos.to_hex().distance_to(tile.to_hex()) as u32
        }
        _ => match task {
            CivilianTask::MoveTo { target } | CivilianTask::BuildRailTo { target } => {
                civilian_pos.to_hex().distance_to(target.to_hex()) as u32
            }
            _ => 0,
        },
    }
...
fn plan_engineer_rail_task(
    ...
    if let Some(next_tile) = find_step_toward(
        depot_frontier,
        bridgehead,
        &nation.owned_tiles,
        avoid_tracker,
...
/// Find the next step from `from` toward `to`, constrained to `allowed_tiles` and avoiding
/// currently occupied tiles.
fn find_step_toward(
    from: TilePos,
    to: TilePos,
...
mod tests {
    ...
    fn test_prospector_pathfinding() {
        ...
        let civilians = vec![crate::ai::snapshot::CivilianSnapshot {
        ...
        assert!(
            matches!(task, Some(CivilianTask::MoveTo { target: t }) if *t == expected_step),
            "Expected move to step (0,1), got {:?}",
            task
        );
    }
}
